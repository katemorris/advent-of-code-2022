...
def adjacent_tree_check(
    ...
    while tree_row > 0 and tree_row < row_max and tree_col > 0 and tree_col < col_max:
        ...
        # print(f"adjacent tree {adj} against {prev_tree_value}")
        if adj < tree_value:
            # print("checking next tree")
...
for row in range(row_num):
    ...
    for column in range(column_num):
        ...
        if above or below or right or left:
            print(f"+1 tree location ({row}, {column}) and value {tree} counts!")
            count += 1
